...
// 递归遍历目录path下的文件，返回后缀名包含suffix的文件名路径
bool GetFileList(const std::string &path, const std::string &suffix,
  ...
  // 定义一各递归地遍历一个目录和子目录，也就是迭代整个目录结构下的所有文件的迭代器
  boost::filesystem::recursive_directory_iterator itr(path);
  // recursive_directory_iterator()表示"end" iterator
  while (itr != boost::filesystem::recursive_directory_iterator()) {
    try {
      // itr->path().string() 表示目录下文件路径
      // absl::EndsWith:用于判断字符串是否以指定后缀结尾,如果以指定后缀结尾返回True
